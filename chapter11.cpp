...
bool chapter11(const char* filename)
{
	int nx = 800;
	int ny = 400;
	int ns = 20;
	...
	// camera axes
	float vfov = float(M_PI) * 0.25f;
	thinLensCamera cam = thinLensCamera::fromLookAt(vec3(3, 3, -2), vec3(0, 0, 1), vec3(0, 1, 0), camera::hfovFromAspect(vfov, aspectRatio), vfov, 1.0f, 0.01f);
	//camera cam = camera::fromLookAt(vec3(3, 3, -2), vec3(0, 0, 1), vec3(0, 1, 0), camera::hfovFromAspect(vfov, aspectRatio), vfov);
