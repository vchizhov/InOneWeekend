...
bool chapter10(const char* filename)
{
	int nx = 800;
	int ny = 400;
	int ns = 20;
	...
	camera cam = camera::fromLookAt(vec3(-2, 2, -1), vec3(0, 0, 1), vec3(0, 1, 0), camera::hfovFromAspect(vfov, aspectRatio), vfov);
	//camera cam(vec3(0.0f), vec3(1.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f), vec3(0.0f, 0.0f, 1.0f), aspectRatio);
